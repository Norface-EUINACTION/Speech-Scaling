...
class Embeddings(object):
	...
	def load_cc_embeddings(self, filepath, limit=200_000, skip_first_line=True):
		"""
        :param filepath: str, path to pretrained word embeddings
        :param limit: int, restrict vocabulary to top-limit words
        """
		word2id = {}
		offset = 0
		with open(filepath, 'r') as f:
			...
			for idx, line in enumerate(f):
				...
				if token not in word2id:
					...
					word2id[token] = offset
					offset += 1
		emb = np.asarray(np.stack(embeddings))
		emb_size = emb.shape[-1]
		self.lang_embeddings["default"] = emb
		self.emb_sizes["default"] = emb_size
		self.lang_emb_norms["default"] = np.linalg.norm(emb, 2, -1)
		self.lang_vocabularies["default"] = word2id