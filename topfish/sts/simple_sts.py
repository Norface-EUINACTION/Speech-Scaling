...
def simple_tokenize(text, stopwords, lower=False, lang_prefix=None):
    ...
    punctuation = [".", ",", "!", ":", "?", ";", "-", ")", "(", "[", "]", "{", "}", "...", "/", "\\", "''", "\"", "'"]
    toks = [(x.strip().lower() if lower else x.strip()) for x in nltk.word_tokenize(text) if
            x.strip().lower() not in stopwords and x.strip().lower() not in punctuation]
...
def fast_cosine_similarity(agg_vecs: np.array, filenames):

    # Compute L2-norms beforehand
    agg_vecs = agg_vecs / np.linalg.norm(agg_vecs, axis=1, keepdims=True)

    @nb.njit()
    def numba_cosine_similarity(agg_vecs):
        N = agg_vecs.shape[0]
        M = np.zeros((N, N))
    ...
    M = numba_cosine_similarity(agg_vecs)
