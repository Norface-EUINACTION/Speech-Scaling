...
def common_component_removal(sent_emb, pc: int = 1) -> np.ndarray:
    """
    Compute and remove the common component across sentence embeddings.
    The common component denotes the primary shared direction across sentence embeddings, that, for instance, largely arises from shared syntax and discourse
    We subsequently remove the projection (i.e. sent_emb @ pc) onto the common component from a sentence embedding.
    CCR can also be viewed as a form of denoising.
    """
...
def map_lang(lang):
    if lang.lower() == 'english':
        ...
        return 'es'
    elif lang.lower() == 'hungary':
        return 'hu'
    elif lang.lower() in ["en", "es", "de", "fr", "it", "hu"]:
        return lang.lower()
    else:
        return None
...
def inverse_map_lang(lang):
    if lang.lower() == 'en':
        ...
        return 'hungary'
    elif lang.lower() in ["english", "spanish", "german", "french", "italian", "hungary"]:
        return lang.lower()
    else:
        return None
...
def eval_func(golds, preds, params=None):
    ...
    conf_matrix = confusion_matrix.ConfusionMatrix(params["dist_labels"], pred_labs, gold_labs, False,
                                                   class_indices=True)
...
def train_cnn(texts, languages, labels, embeddings, parameters, model_serialization_path, emb_lang='default'):
    ...
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '  Encoding languages (full name to abbreviation)...',
          flush=True)
    langs = [map_lang(x) for x in languages]
    # preparing training examples
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '  Preparing training examples...', flush=True)
    x_train, y_train, dist_labels = data_shaper.prep_classification(texts_clean, labels, embeddings,
                                                                    embeddings_language=emb_lang,
                                                                    multilingual_langs=langs, numbers_token='<NUM/>',
                                                                    punct_token='<PUNC/>',
                                                                    add_out_of_vocabulary_terms=False)

    # defining the CNN model
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '  Defining the CNN model...', flush=True)
    cnn_classifier = cnn.CNN(embeddings=(embeddings.emb_sizes[emb_lang], embeddings.lang_embeddings[emb_lang]),
                             num_conv_layers=parameters["num_convolutions"], filters=parameters["filters"],
                             k_max_pools=parameters["k_max_pools"], manual_features_size=0)
    cnn_classifier.define_model(len(x_train[0]), len(dist_labels), loss_functions.softmax_cross_entropy,
                                len(embeddings.lang_vocabularies[emb_lang]), l2_reg_factor=parameters["reg_factor"],
    ...
    simp_trainer = trainer.SimpleTrainer(cnn_classifier, session, build_feed_dict_func, eval_func,
                                         configuration_func=None)
    simp_trainer.train(list(zip(x_train, y_train)), parameters["batch_size"], parameters["num_epochs"],
                       num_epochs_not_better_end=5, epoch_diff_smaller_end=parameters["epoch_diff_smaller_end"],
                       print_batch_losses=True, eval_params={"dist_labels": dist_labels})
...
def test_cnn(texts, languages, labels, embeddings, model_serialization_path, predictions_file_path, parameters,
             emb_lang='default'):
    # loading the serialized
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '  Loading the serialized model...', flush=True)
    cnn_classifier, session = cnn.load_model(model_serialization_path, embeddings.lang_embeddings[emb_lang],
                                             loss_functions.softmax_cross_entropy, just_predict=(labels is None))
    ...
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '  Preparing training examples...', flush=True)
    if labels:
        x_test, y_test, dist_labels = data_shaper.prep_classification(texts_clean, labels, embeddings,
                                                                      embeddings_language=emb_lang,
                                                                      multilingual_langs=langs, numbers_token='<NUM/>',
                                                                      punct_token='<PUNC/>',
                                                                      add_out_of_vocabulary_terms=False,
        ...
        x_test = data_shaper.prep_classification(texts_clean, labels, embeddings, embeddings_language=emb_lang,
                                                 multilingual_langs=langs, numbers_token='<NUM/>',
                                                 punct_token='<PUNC/>', add_out_of_vocabulary_terms=False,
                                                 dist_labels=cnn_classifier.dist_labels,
                                                 max_seq_len=cnn_classifier.max_text_length)

    simp_trainer = trainer.SimpleTrainer(cnn_classifier, session, build_feed_dict_func,
                                         None if not labels else eval_func, configuration_func=None)
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '  Starting test...', flush=True)
    results = simp_trainer.test(list(zip(x_test, y_test if labels else [None] * len(x_test))), parameters["batch_size"],
                                eval_params={"dist_labels": cnn_classifier.dist_labels}, batch_size_irrelevant=True)
...
def scale_efficient(filenames, texts, languages, embeddings, predictions_file_path, parameters, emb_lang='default',
                    stopwords=[]):
    ...
    #todo: Coded against personal workspace
    ...
    with open("/work-ceph/mbui/eia-crawling/eia_crawling/sim_pairs_accelerated.csv", "w") as f:
        writer = csv.writer(f)
        for row in pairs:
            writer.writerow(row)
...
def scale(filenames, texts, languages, embeddings, predictions_file_path, parameters, emb_lang='default'):
    """Scaling with pre-trained embeddings (e.g. FastText embeddings).
    ...
    texts_tokenized = []
    for i in range(len(texts)):
        print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + "  Document " + str(i + 1) + " of " + str(len(texts)),
              flush=True)
    ...
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + "  Computing similarities between document pairs...",
          flush=True)
    ...
    for i in range(len(items) - 1):
        for j in range(i + 1, len(items)):
            cntr += 1
            print("Pair: " + items[i][0] + " - " + items[j][0] + " (" + str(cntr) + " of " + str(
                (len(items) * (len(items) - 1)) / 2), flush=True)

            sim = simple_sts.greedy_alignment_similarity(embeddings, items[i][2], items[j][2], lowest_sim=0.01,
                                                         length_factor=0.01)
    ...
    if predictions_file_path:
        ...
        print(f'No path has been found {predictions_file_path}')
        print(predictions_file_path)
    return scores


def scale_supervised(filenames, texts, languages, embeddings, predictions_file_path, pivot1, pivot2, stopwords):
    """Scaling with pre-trained embeddings (e.g. FastText embeddings).
...
def topically_scale(filenames, texts, languages, embeddings, model_serialization_path, predictions_file_path,
                    parameters, emb_lang='default', stopwords=[]):
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + "  Loading classifier...", flush=True)
    cnn_classifier, session = cnn.load_model(model_serialization_path, embeddings.lang_embeddings[emb_lang],
                                             loss_functions.softmax_cross_entropy, just_predict=True)
    ...
    for item in items:
        ...
        # preparing training examples
        x_test = data_shaper.prep_classification(sents_clean, None, embeddings, embeddings_language=emb_lang,
                                                 multilingual_langs=langs, numbers_token='<NUM/>',
                                                 punct_token='<PUNC/>', add_out_of_vocabulary_terms=False,
                                                 dist_labels=cnn_classifier.dist_labels,
                                                 max_seq_len=cnn_classifier.max_text_length)

        results = simp_trainer.test(list(zip(x_test, [None] * len(x_test))), parameters["batch_size"],
                                    batch_size_irrelevant=True, print_batches=True)
    ...
    for l in cnn_classifier.dist_labels:
        label_filtered = [(fn, classified_texts[fn][0][2],
                           ' '.join([sent_label[0] for sent_label in classified_texts[fn] if sent_label[1] == l])) for
                          fn in classified_texts]
        ...
        if len(label_filtered) > 3:
            ...
            for i in range(len(fns)):
                io_helper.write_list(
                    os.path.dirname(predictions_file_path) + "/" + fns[i].split(".")[0] + "_" + l.replace(" ",
                                                                                                          "-") + ".txt",
                    [filt_texts[i]])

            label_scale = scale_efficient(fns, filt_texts, [inverse_map_lang(x) for x in langs], embeddings, None,
                                          parameters, emb_lang=emb_lang, stopwords=stopwords)
            lines_to_write.append("Scaling for class: " + l)
            lines_to_write.extend([k + " " + str(label_scale[k]) for k in label_scale])
            lines_to_write.append("\n")
        else:
            lines_to_write.append(
                "Topic: " + l + ": Insufficient number of files contains text of this topic (i.e., class) in order to allow for scaling for the topic.")
            print(
                "Topic: " + l + ": Insufficient number of files contains text of this topic (i.e., class) in order to allow for scaling for the topic.",
                flush=True)
